 ...
 */
async function buildRequestedFields(submitterDid, searchHandle, requestFields, values, from, to) {
    const schemas = {};
    ...
    for (const referent of Object.keys(requestFields)) {
        ...
        if (result.length > 0) {
            ...
            if (credential.rev_reg_id) {
                ...
                const filePath = path.join(blobStorage.config.base_dir, revRegDef.value.tailsHash);
...
module.exports = {
     ...
     */
    async buildRequest(walletHandle, recipientDid, proofRequest) {
     ...
     * @param {object} [values]
     * @return {Promise<object>} proof object
     */
    async create(walletHandle, masterSecretId, myDid, proofRequest, values = {}) {
        ...
        ] = await buildRequestedFields(myDid, searchHandle, proofRequest.requested_attributes, values, from, to);
        ...
        ] = await buildRequestedFields(myDid, searchHandle, proofRequest.requested_predicates, values, from, to);
     ...
     * Verify a proof
     * @param {string} submitterDid
     * @param {string} request proof request
     * @param {object} proof
     * @return {Promise<boolean>} isValid
     */
    async verify(submitterDid, request, proof) {
        // fetch verification-related information from ledger
        const [schemas, credentialDefinitions, revRegDefs, revRegs] = await ledger.verifierGetEntitiesFromLedger(
            submitterDid,
            proof.identifiers
        );
        // verify proof
        return await sdk.verifierVerifyProof(request, proof, schemas, credentialDefinitions, revRegDefs, revRegs);
    }
};
