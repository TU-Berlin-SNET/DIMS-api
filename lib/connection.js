...
module.exports = {
     ...
     */
    async buildOffer(did, vk, endpoint) {
     ...
     */
    async buildRequest(did, vk, myDid, myVk, myEndpoint, offerNonce) {
     ...
     */
    buildResponse(myDid, myVk, theirDid, requestNonce) {
        return {
            id: requestNonce,
            aud: theirDid,
     ...
     */
    async store(walletHandle, myDid, theirDid, theirVk, meta) {
        try {
            await sdk.storeTheirDid(walletHandle, {
                did: theirDid,
                verkey: theirVk
            });
        } catch (err) {
            ...
            // so ignore
            if (err.name !== 'IndyError' || err.message !== '213') {
                throw err;
            }
        }
        await sdk.createPairwise(walletHandle, theirDid, myDid, JSON.stringify(meta));
    }
};
